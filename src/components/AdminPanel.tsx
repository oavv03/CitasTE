...
export default function AdminPanel({ citas, onUpdateCitas, onClose }: AdminPanelProps) {
  ...
  // WhatsApp floating button config states
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.success && data.config) {
          setWhatsappNumber(data.config.numero || '');
    ...
    try {
      ...
      const res = await fetch('/api/whatsapp/config', {
        ...
        body: JSON.stringify({
          ...
          habilitado: whatsappEnabled
        })
      });
      if (res.ok) {
        const data = await res.json();
        if (data.success) {
          setWhatsappMessageStatus('config_success');
          // Notify App component or other listeners
          if (typeof window !== 'undefined') {
            window.dispatchEvent(new CustomEvent('whatsapp_config_changed'));
          }
        } else {
          setWhatsappMessageStatus('config_error');
        }
      } else {
        setWhatsappMessageStatus('config_error');
      }
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex flex-col gap-3 bg-slate-900/60 p-4 rounded-lg border border-slate-800">
                      <div className="flex justify-between items-start gap-4">
                        <div>
                          <span className="text-[11px] font-extrabold uppercase text-slate-250 block">Botón Flotante de WhatsApp</span>
                          <span className="text-[10px] text-slate-450">Agregue un icono flotante a la derecha para que los ciudadanos abran un chat con soporte directo en un clic</span>
                        </div>
                        <input
                          type="checkbox"
                          disabled={currentRole !== 'super'}
                          ...
                          className="rounded cursor-pointer accent-green-600 w-4 h-4 disabled:opacity-50 disabled:cursor-not-allowed"
                        />
                      </div>

                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-1">
                        <div className="space-y-1">
                          <label className="text-[9.5px] font-black uppercase text-slate-400 tracking-wider block">Número de WhatsApp (con Código de País)</label>
                          <input
                            type="text"
                            disabled={currentRole !== 'super'}
                            value={whatsappNumber}
                            onChange={(e) => setWhatsappNumber(e.target.value)}
                            placeholder="Ej: 50766000000"
                            className="w-full bg-slate-950 border border-slate-800 text-slate-200 p-2 rounded text-xs px-3 focus:outline-none focus:border-green-600 disabled:opacity-50"
                          />
                          <p className="text-[9px] text-slate-500">Ingrese el número pegado (sin espacios, guiones ni símbolo +). Ej: 50766554433</p>
                        </div>

                        <div className="space-y-1">
                          <label className="text-[9.5px] font-black uppercase text-slate-400 tracking-wider block">Mensaje Inicial Predeterminado</label>
                          <input
                            type="text"
                            disabled={currentRole !== 'super'}
                            value={whatsappMessage}
                            onChange={(e) => setWhatsappMessage(e.target.value)}
                            placeholder="Ej: Hola, solicito asistencia..."
                            className="w-full bg-slate-950 border border-slate-800 text-slate-200 p-2 rounded text-xs px-3 focus:outline-none focus:border-green-600 disabled:opacity-50"
                          />
                          <p className="text-[9px] text-slate-500">Mensaje automático precargado en el chat del ciudadano.</p>
                        </div>
                      </div>

                      {currentRole === 'super' && (
                        <div className="flex items-center justify-between gap-3 pt-2.5 border-t border-slate-950">
                          <div>
                            {whatsappMessageStatus === 'config_success' && (
                              <span className="text-[10px] text-green-500 font-extrabold uppercase">¡Configuración guardada exitosamente!</span>
                            )}
                            {whatsappMessageStatus === 'config_error' && (
                              <span className="text-[10px] text-red-500 font-extrabold uppercase">Error al guardar la configuración.</span>
                            )}
                          </div>
                          <button
                            type="button"
                            ...
                            className="bg-green-600 hover:bg-green-700 disabled:opacity-50 text-white font-extrabold text-[11px] uppercase tracking-wider px-5 py-2 rounded transition cursor-pointer"
                          >
                            {whatsappSaving ? 'Guardando...' : 'Guardar Ajustes de WhatsApp'}
                          </button>
                        </div>
                      )}
                    </div>
